...
class util_sys_minifyhtml
{
     ...
     * @var string
     */
    protected static $className = 'util_sys_minifyhtml';
     ...
     * @return string
     */
    static function minify($html, $options = array())
    {
        ...
        $html = preg_replace_callback('/(\\s*)(<script\\b[^>]*?>)([\\s\\S]*?)<\\/script>(\\s*)/i', array(
            self::$className,
            '_removeScriptCB'
        ), $html);
        
        // replace STYLEs (and minify) with placeholders
        $html = preg_replace_callback('/\\s*(<style\\b[^>]*?>)([\\s\\S]*?)<\\/style>\\s*/i', array(
            self::$className,
            '_removeStyleCB'
        ), $html);
        
        // remove HTML comments (not containing IE conditional comments).
        $html = preg_replace_callback('/<!--([\\s\\S]*?)-->/', array(
            self::$className,
            '_commentCB'
        ), $html);
        
        // replace PREs with placeholders
        $html = preg_replace_callback('/\\s*(<pre\\b[^>]*?>[\\s\\S]*?<\\/pre>)\\s*/i', array(
            self::$className,
            '_removePreCB'
        ), $html);
        
        // replace TEXTAREAs with placeholders
        $html = preg_replace_callback('/\\s*(<textarea\\b[^>]*?>[\\s\\S]*?<\\/textarea>)\\s*/i', array(
            self::$className,
            '_removeTaCB'
        ), $html);
    ...
    protected static function _removeScriptCB($m)
    {
        ...
        return self::_reservePlace(self::_needsCdata($js) ? "{$ws1}{$openScript}/*<![CDATA[*/{$js}/*]]>*/</script>{$ws2}" : "{$ws1}{$openScript}{$js}</script>{$ws2}");
    }

    protected static function _removeCdata($s)
    {
        return (false !== strpos($s, '<![CDATA[')) ? str_replace(array(
            '<![CDATA[',
            ']]>'
        ), '', $s) : $s;
    }

    protected static function _needsCdata($s)
    {
        return (self::$_isXhtml && preg_match('/(?:[<&]|\\-\\-|\\]\\]>)/', $s));
    }
}